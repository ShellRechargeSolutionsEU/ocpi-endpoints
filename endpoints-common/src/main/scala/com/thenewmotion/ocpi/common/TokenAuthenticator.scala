...
class TokenAuthenticator(toApiUser: TheirToken => Future[Option[GlobalPartyId]])(implicit executionContext: ExecutionContext)
  ...
  override def apply(credentials: Option[HttpCredentials]): Future[AuthenticationResult[GlobalPartyId]] = {
    credentials
      .flatMap {
        ...
        case Some(x) => toApiUser(TheirToken(x)).map {
