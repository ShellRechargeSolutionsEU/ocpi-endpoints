...
class CpoTokensRoute(
  ...
  implicit def tokenErrorResp(implicit errorMarshaller: ToResponseMarshaller[(StatusCode, ErrorResp)]): ToResponseMarshaller[TokenError] = {
    errorMarshaller.compose[TokenError] { tokenError =>
  ...
  def route(apiUser: ApiUser)(implicit executionContext: ExecutionContext) = handleRejections(AuthorizationRejectionHandler.Default) {
