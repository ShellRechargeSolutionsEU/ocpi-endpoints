...
object Errors{
  ...
  case class SelectedVersionNotHostedByThem(
    reason: Option[String] = Some("Selected version not supported by the party systems.")) extends HandshakeError
  ...
  case class CouldNotPersistNewCredsForUs(
    reason: Option[String] = Some("Could not persist new credentials set to us.")) extends HandshakeError
  case class CouldNotPersistNewToken(
    reason: Option[String] = Some("Could not persist new token.")) extends HandshakeError
